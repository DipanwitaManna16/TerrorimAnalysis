...
def create_app_ui():
    ...
    dcc.Dropdown(
          ...
          multi=True    #MultipleSelection from the dropdown
    ),
    dcc.Dropdown(
          id='date', 
          options=date_list,
...
def update_app_ui(month_value, date_value,region_value,country_value,state_value,city_value,attack_value,year_value):
  ...
  if month_value is None:
      ...
      if date_value is None:
          df1 = df1[df1['imonth'] == month_value]
      else:
          df1 = df1 [ (df1['imonth'] == month_value ) & ( df1['iday'] == date_value ) ]
  ...
  if region_value is None:
      ...
      if country_value is None:
          df1 = df1[df1['region_txt'] == region_value]
      else:
          if state_value is None:
              df1 = df1 [ ( df1["region_txt"]==region_value )  & (df1['country_txt'] == country_value) ]
          else:
              if city_value is None:
                  df1 = df1[ (df1['region_txt']==region_value) & (df1['country_txt']==country_value) & (df1['provstate']==state_value) ]
              else:
                  df1 = df1[ (df1['region_txt']==region_value) & (df1['country_txt']==country_value) & (df1['provstate']==state_value) & (df1['city']==city_value) ]
                  
   #AttackType
  if attack_value is None:
      pass
  else:
      df1 = df1[ df1['attacktype1_txt']==attack_value ]
