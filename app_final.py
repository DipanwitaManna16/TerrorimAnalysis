...
import dash_html_components as html     #components of dash
from dash.dependencies import Input, State, Output 
import dash_core_components as dcc 
#to generate figures
import plotly.graph_objects as go  
#to generate charts and maps
...
def load_data():
  ...
  #We need a dictionary for RangeSlider
  global year_dict
  year_dict = {str(year): str(year) for year in year_list}
  
  #Chart dropdown options
  global chart_dropdown_values
  chart_dropdown = {"Terrorist Organisation":'gname', 
  ...
  chart_dropdown_values = [{"label":keys, "value":value} for keys, value in chart_dropdown.items()]
...
# Layout of the page
# a method  which makes the ui for the website
def create_app_ui():
  ...
  html.H1( id='Main_title', children='Terrorism Analysis and Finding Insights' ,
          style={'textAlign': 'center','backgroundColor':'#40E0D0'}),
  
  #Tabs
  dcc.Tabs(id="Tabs", value="Map",children=[
      dcc.Tab(label="Map Tool" ,id="Map Tool",value="Map", style=tab_style ,children=[
          dcc.Tabs(id = "subtabs1", value = "WorldMap",  children = [
              dcc.Tab(label="World Map Tool", id="World", value="WorldMap", selected_style=tab_selected_style),
              dcc.Tab(label="India Map Tool", id="India", value="IndiaMap" ,selected_style=tab_selected_style)
              ]),
         html.Br(), 
         #All the dropdowns and RangeSlider
         dcc.Dropdown(
              id='month', 
                options=month_list,
          ...
          html.H3('Select the Year Range from the Slider', id='year_title'),
          dcc.RangeSlider(
                    id='year-slider',
                    min=min(year_list),
                    max=max(year_list),
      ...
      dcc.Tab(label = "Chart Tool", id="chart tool", value="Chart", style=tab_style , children=[
          dcc.Tabs(id = "subtabs2", value = "WorldChart",children = [
              dcc.Tab(label="World Chart Tool", id="WorldC", value="WorldChart" , selected_style=tab_selected_style),          
            dcc.Tab(label="India Chart Tool", id="IndiaC", value="IndiaChart" , selected_style=tab_selected_style)]),
            html.Br(),
            dcc.Dropdown(id="Chart_Dropdown", options = chart_dropdown_values, placeholder="Select option",
                         value = "region_txt",style={'border-radius':'40px'},), 
            html.Br(),
            html.Br(),
            html.Hr(),
            dcc.Input(id="search", placeholder="Search Filter" , style={'border-radius':'40px' , 'width':'40%'}),
            html.Hr(),
            html.Br(),
            
            html.H3('Select the Year Range from the Slider', id='cyear_title'),
            dcc.RangeSlider(
                    id='cyear_slider',
                    min=min(year_list),
                    max=max(year_list),
...
def update_app9_ui(Tabs, month_value, date_value,region_value,country_value,state_value,city_value,attack_value,year_value,
    ...
    if Tabs == "Map":
        ...
        if attack_value == [] or attack_value is None:
         ...
         # set the figure for blank, since this callback 
...
def main():
  ...
  print("Welcome to the project of Terrorism Analysis") 
  ...
  app.title = "Terrorism Analysis and Finding Insights"      #title of the website
  
  # to start the app to run
  app.run_server()         # debug=True

  print("Thank You")
  
  #deallocating the spaces
